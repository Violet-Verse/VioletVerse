    ...
    if (user?.addr) {
      const getAccountBalance = async () => {
        const result = await fcl
          .send([fcl.script(getBalance), fcl.args([fcl.arg(user?.addr, types.Address)])])
          .then(fcl.decode)
    ...
    >
      {loggedIn ? (
        <Grid container direction="column" spacing={5} sx={{ mt: 5 }}>
          <Grid item>
            <h4>Total Supply: {(Math.round(totalSupply * 100) / 100).toFixed(2)}</h4>
          </Grid>

          <Grid item>
            <h4>Your Balance: {(Math.round(userBalance * 100) / 100).toFixed(2)}</h4>
          </Grid>
        </Grid>
      ) : (
        <>
