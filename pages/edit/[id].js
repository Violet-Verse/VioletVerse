...
export async function getServerSideProps(context) {
  ...
  const postData = await collection.find({ _id: new ObjectId(postId) }).toArray()

  if (!postData || postData.length === 0) {
    return { notFound: true, props: { posts: {} } }
  }

  const lastEditor = (await getUserByIssuer(postData?.[0]?.lastEditedBy)) ?? null
...
const EditArticle = ({ posts, lastEditor }) => {
  const fetchAuthor = (url, id) => fetch(`${url}?id=${id}`).then((r) => r.json())
  ...
  return (
    <PostEditor data={posts} editorMode author={author?.user} lastEditor={lastEditor} />
  )
}
