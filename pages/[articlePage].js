...
import { getAuthorForPost, getContributorForPost } from './api/database/getUserForPost'
...
const Article = ({ posts, allPosts, authorData, tokenGatePrice, contributorData }) => {
  ...
  const noTokenGateAccess =
    tokenGatePrice && user && purchasedArray.indexOf(posts._id.toString()) === -1
  ...
  const addTokenWhitelist = async () => {
    ...
    try {
      ...
      const transactionId = await fcl.mutate({
        ...
        args: (arg, t) => [
          arg(contributor?.flowAddress || author?.flowAddress, types.Address),
        ...
        >
          ...
          {contributorData && (
            ...
            >
                  ...
                  <Tooltip title={contributor ? 'Community Contributor' : ''}>
                    <Link href={`/user/${contributor?.username || author?.username}`}>
                      <a>
                        ...
                        >
                          BY{' '}
                          {contributor?.name.toUpperCase() || author?.name.toUpperCase()}
                        </p>
                      </a>
                    </Link>
                  </Tooltip>
              ...
              >
                <Image alt="star" src="/star.svg" height={20} width={20} />
              </Grid>
              <Grid
                item
                sx={{
          ...
          >
            ...
            >
              This content is token gated for <b>{tokenGatePrice} $VV Tokens</b>.
            </p>
                ...
                <Tooltip title={contributor ? 'Community Contributor' : ''}>
                  <Link href={`/user/${contributor?.username || author?.username}`}>
                    <a>
                      ...
                      >
                        BY {contributor?.name.toUpperCase() || author?.name.toUpperCase()}
                      </p>
                    </a>
                  </Link>
                </Tooltip>
